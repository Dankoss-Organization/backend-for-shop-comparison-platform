...
### Benchmark parallel CPU signal

For measuring execution time of the worker-thread algorithm with different input sizes,
thread pool sizes, iterations, and task counts, run:

```bash
npm run bench:worker-threads
```

You can override default benchmark dimensions via environment variables:

- `BENCH_POOL_SIZES` (example: `1,2,4,8`)
- `BENCH_DATA_SIZES` (example: `16,128,512,1024`)
- `BENCH_ITERATIONS` (example: `5000,20000,40000`)
- `BENCH_TASK_COUNTS` (example: `1,4,16,32`)

PowerShell example:

```powershell
$env:BENCH_POOL_SIZES="1,2,4,8"
$env:BENCH_DATA_SIZES="64,256,1024"
$env:BENCH_ITERATIONS="10000,30000"
$env:BENCH_TASK_COUNTS="1,8,32"
npm run bench:worker-threads
```
